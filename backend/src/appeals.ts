...
type GeminiRecommendation = {
  recommendation: "restore" | "deny" | "human_review";
  confidence: number;
  rationale: string;
};
...
const receiptHash = (token: string) => createHash("sha256").update(token).digest("hex");

export function createAppeal(userId: number, statement: string, networkHash: string) {
  ...
  if (existing) return { error: "An appeal for this account is already awaiting review." } as const;
  ...
  return { id: Number(result.lastInsertRowid), receipt } as const;
}

export function publicAppealStatus(id: string, receipt: string) {
  return db.prepare(`
...
function appealContext(id: number) {
  return db.prepare(`
  ...
  `).get(id) as { id: number; statement: string; banReason: string | null; moderationHistory: string } | undefined;
}

async function askGemini(id: number): Promise<GeminiRecommendation> {
  if (!apiKey) return { recommendation: "human_review", confidence: 0, rationale: "Gemini is not configured. A moderator must review this appeal." };
  const context = appealContext(id);
  if (!context) throw new Error("Appeal not found.");
  ...
  try {
    ...
    if (!text) throw new Error("Gemini did not return a recommendation.");
    const result = JSON.parse(text) as Partial<GeminiRecommendation>;
    if (!["restore", "deny", "human_review"].includes(String(result.recommendation)) || !Number.isFinite(result.confidence) || typeof result.rationale !== "string") throw new Error("Gemini returned an invalid recommendation.");
    return { recommendation: result.recommendation as GeminiRecommendation["recommendation"], confidence: Math.min(1, Math.max(0, Number(result.confidence))), rationale: result.rationale.slice(0, 1_000) };
  } finally {
    clearTimeout(timer);
  }
}

export async function evaluateAppeal(id: number) {
  try {
    ...
    db.prepare("UPDATE moderation_appeals SET status = 'error', rationale = ? WHERE id = ? AND status = 'pending'")
      .run((error as Error).message.slice(0, 1_000), id);
  }
}
...
export function resolveAppeal(id: string, status: "restored" | "denied") {
