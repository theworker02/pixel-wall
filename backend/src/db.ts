...
const historyColumns = db.prepare("PRAGMA table_info(pixel_history)").all() as Array<{ name: string }>;
...
const moderationColumns = db.prepare("PRAGMA table_info(moderation_events)").all() as Array<{ name: string }>;
