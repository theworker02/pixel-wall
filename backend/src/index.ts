...
import { auth, createToken, deleteToken, requireAuth, type AuthedRequest } from "./auth.js";
...
const drawTimes = new Map<number, number>();
...
const moderatorToken = (authorization?: string) => authorization?.replace(/^Bearer /, "");
const isModeratorRequest = (req: express.Request) => req.path.startsWith("/api/moderation/") && safeEqual(moderatorToken(req.headers.authorization), process.env.MODERATION_ADMIN_KEY);
const isAppealRequest = (req: express.Request) => req.path === "/api/appeals" || /^\/api\/appeals\/\d+$/.test(req.path);
...
const validCoord = (value: unknown): value is number => Number.isInteger(value) && Number(value) >= 0 && Number(value) < CANVAS_SIZE;
const publicUser = (user: DbUser) => ({ ...user, canvasSize: CANVAS_SIZE });
...
const entryFor = (x: number, y: number): CanvasEntry => ({ origin_x: Math.floor(x / ENTRY_SIZE) * ENTRY_SIZE, origin_y: Math.floor(y / ENTRY_SIZE) * ENTRY_SIZE, size: ENTRY_SIZE });
const publicEntry = (entry?: CanvasEntry) => entry ? { originX: entry.origin_x, originY: entry.origin_y, size: entry.size } : null;
...
app.post("/api/auth/register", authLimit, async (req, res) => {
  ...
  if (!/^[a-z0-9_]{3,20}$/i.test(username)) return res.status(400).json({ error: "Username must use 3-20 letters, numbers, or underscores." });
  ...
  if (blacklisted("email", fingerprint("email", email)) || blacklisted("username", fingerprint("username", username))) return res.status(403).json({ error: "Access denied." });
  try {
    ...
    const user = db.prepare("SELECT id, username, created_at, last_active FROM users WHERE id = ?").get(result.lastInsertRowid) as DbUser;
...
app.post("/api/auth/login", authLimit, async (req, res) => {
  ...
  const password = String(body.password ?? "");
  const row = db.prepare("SELECT * FROM users WHERE email = ? OR username = ?").get(identifier, identifier) as (DbUser & { password_hash: string; email: string }) | undefined;
  const passwordMatches = password.length <= 128 && await bcrypt.compare(password, row?.password_hash ?? DUMMY_PASSWORD_HASH);
  if (!row || !passwordMatches) return res.status(401).json({ error: "Incorrect email, username, or password." });
  if (row.banned_at || blacklisted("email", fingerprint("email", row.email)) || blacklisted("username", fingerprint("username", row.username))) return res.status(403).json({ error: "Access denied." });
  const { password_hash: _, email: __, ...user } = row;
  res.json({ token: createToken(user.id), user: publicUser(user) });
});
app.get("/api/auth/me", (req: AuthedRequest, res) => res.json({ user: req.user ? publicUser(req.user) : null }));
...
app.post("/api/appeals", appealLimit, async (req, res) => {
  ...
  if (statement.length < 30 || statement.length > 1_500) return res.status(400).json({ error: "Appeal statement must use 30-1500 characters." });
  const row = db.prepare("SELECT id, password_hash, banned_at FROM users WHERE email = ? OR username = ?").get(identifier, identifier) as { id: number; password_hash: string; banned_at: string | null } | undefined;
  const passwordMatches = password.length <= 128 && await bcrypt.compare(password, row?.password_hash ?? DUMMY_PASSWORD_HASH);
  if (!row || !passwordMatches || !row.banned_at) return res.status(401).json({ error: "Unable to verify a banned account with those credentials." });
  const appeal = createAppeal(row.id, statement, networkFingerprint(req));
  if ("error" in appeal) return res.status(409).json({ error: appeal.error });
...
app.get("/api/appeals/:id", appealLimit, (req, res) => {
  ...
  if (!receipt) return res.status(400).json({ error: "Appeal receipt required." });
...
app.get("/api/canvas", (_req, res) => {
  ...
  `).all().map((batch) => ({ ...batch as object, pixels: JSON.parse((batch as { pixels: string }).pixels) }));
  res.json({ width: CANVAS_SIZE, height: CANVAS_SIZE, pixels, recent });
});
app.get("/api/canvas/entry", requireAuth, (req: AuthedRequest, res) => {
  const entry = db.prepare("SELECT origin_x, origin_y, size FROM canvas_entries WHERE user_id = ?").get(req.user!.id) as CanvasEntry | undefined;
  res.json({ entry: publicEntry(entry), maxEntrySize: ENTRY_SIZE });
});

app.post("/api/canvas/pixels", requireAuth, (req: AuthedRequest, res) => {
  const user = req.user!;
  const body = req.body && typeof req.body === "object" ? req.body : {};
  const points = body.pixels as unknown;
  if (!Array.isArray(points) || points.length < 1) return res.status(400).json({ error: "Send at least one pixel." });
  const age = Date.now() - Date.parse(user.created_at);
  const limit = age < 86_400_000 ? 16 : 64;
  if (points.length > limit) return res.status(400).json({ error: `Your current batch limit is ${limit} pixels.` });
  if (points.some((p) => !p || !validCoord(p.x) || !validCoord(p.y) || (p.erase !== true && !validColor(p.color)))) return res.status(400).json({ error: "Invalid pixel coordinates or color." });
  const unique = [...new Map(points.map((p) => [`${p.x}:${p.y}`, p])).values()];
  const savedEntry = db.prepare("SELECT origin_x, origin_y, size FROM canvas_entries WHERE user_id = ?").get(user.id) as CanvasEntry | undefined;
  if (!savedEntry && unique.every((point) => point.erase === true)) return res.status(400).json({ error: "Draw at least one pixel to claim your entry before using the eraser." });
  const entry = savedEntry ?? entryFor(unique[0].x, unique[0].y);
  if (unique.some((point) => !withinEntry(point, entry))) return res.status(400).json({ error: savedEntry ? "Your account already has one entry. Keep drawing inside your 32 x 32 plot." : "Your first entry must fit inside one 32 x 32 plot." });
  const now = Date.now();
  const wait = COOLDOWN_MS - (now - (drawTimes.get(user.id) ?? 0));
  if (wait > 0) return res.status(429).json({ error: "Cooldown active.", retryAfter: wait });
  ...
  db.exec("BEGIN");
  try {
    if (!savedEntry) db.prepare("INSERT INTO canvas_entries (user_id, origin_x, origin_y, size) VALUES (?, ?, ?, ?)").run(user.id, entry.origin_x, entry.origin_y, entry.size);
    for (const p of unique) {
      const previous = read.get(p.x, p.y) as { color: string; user_id: number } | undefined;
      if (p.erase === true) {
...
app.post("/api/canvas/undo", requireAuth, (req: AuthedRequest, res) => {
  const user = req.user!;
  const latest = db.prepare("SELECT batch_id FROM pixel_history WHERE user_id = ? AND hidden_at IS NULL ORDER BY id DESC LIMIT 1").get(user.id) as { batch_id: string } | undefined;
  if (!latest) return res.status(404).json({ error: "Nothing to undo yet." });
  const rows = db.prepare("SELECT * FROM pixel_history WHERE user_id = ? AND batch_id = ? ORDER BY id DESC").all(user.id, latest.batch_id) as Array<{ x: number; y: number; previous_color: string | null; new_color: string; operation: string; previous_user_id: number | null }>;
  const current = db.prepare("SELECT color, user_id FROM pixels WHERE x = ? AND y = ?");
  const updates: Array<{ x: number; y: number; color: string | null }> = [];
  db.exec("BEGIN");
  try {
    for (const row of rows) {
      const pixel = current.get(row.x, row.y) as { color: string; user_id: number } | undefined;
      if (row.operation === "erase") {
        if (pixel || !row.previous_color) continue;
        db.prepare("INSERT INTO pixels (user_id, x, y, color) VALUES (?, ?, ?, ?)").run(row.previous_user_id ?? user.id, row.x, row.y, row.previous_color);
      } else {
        if (!pixel || pixel.user_id !== user.id || pixel.color !== row.new_color) continue;
        if (row.previous_color) db.prepare("UPDATE pixels SET color = ?, user_id = ?, created_at = CURRENT_TIMESTAMP WHERE x = ? AND y = ?").run(row.previous_color, row.previous_user_id ?? user.id, row.x, row.y);
        else db.prepare("DELETE FROM pixels WHERE x = ? AND y = ?").run(row.x, row.y);
      }
...
const leaderboard = (where = "", order = "pixelsPlaced DESC") =>
  db.prepare(`${leaderboardSelect} ${where} GROUP BY u.id ORDER BY ${order} LIMIT 100`).all();
const longestStreak = (userId: number) => {
  const days = db.prepare("SELECT DISTINCT date(created_at) AS day FROM pixel_history WHERE user_id = ? AND hidden_at IS NULL ORDER BY day").all(userId) as Array<{ day: string }>;
  ...
  for (const { day } of days) {
    ...
    longest = Math.max(longest, current); previous = stamp;
  }
  return longest;
};
...
app.get("/api/leaderboard/streaks", (_req, res) => {
  const users = (leaderboard() as Array<{ id: number }>).map((user) => ({ ...user, streak: longestStreak(user.id) })).sort((a, b) => b.streak - a.streak);
  res.json({ users });
});
...
app.get("/api/users/:id/profile", (req, res) => {
  ...
  res.json({ profile: { ...profile as object, drawingStreak: longestStreak(Number(req.params.id)) }, preview });
});
...
const requireModerator = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  if (!safeEqual(moderatorToken(req.headers.authorization), process.env.MODERATION_ADMIN_KEY)) return res.status(401).json({ error: "Moderator authorization required." });
  next();
};
...
app.post("/api/moderation/events/:id/ban", requireModerator, (req, res) => {
  const event = db.prepare("SELECT id, user_id AS userId, network_hash AS networkHash FROM moderation_events WHERE id = ?").get(String(req.params.id)) as { id: number; userId: number; networkHash: string } | undefined;
  if (!event) return res.status(404).json({ error: "Moderation event not found." });
...
app.post("/api/moderation/events/:id/allow", requireModerator, (req, res) => {
  ...
  if (!result.changes) return res.status(404).json({ error: "Reviewable moderation event not found." });
  res.json({ ok: true });
});
app.post("/api/moderation/users/:id/restore", requireModerator, (req, res) => {
  if (!restoreUserAccess(Number(req.params.id))) return res.status(404).json({ error: "Banned user not found." });
...
app.post("/api/moderation/appeals/:id/restore", requireModerator, (req, res) => {
  const appeal = db.prepare("SELECT user_id AS userId FROM moderation_appeals WHERE id = ? AND status IN ('recommended_restore', 'recommended_deny', 'review', 'error')").get(String(req.params.id)) as { userId: number } | undefined;
  if (!appeal || !restoreUserAccess(appeal.userId) || !resolveAppeal(String(req.params.id), "restored")) return res.status(404).json({ error: "Reviewable appeal not found." });
...
app.post("/api/moderation/appeals/:id/deny", requireModerator, (req, res) => {
  if (!resolveAppeal(String(req.params.id), "denied")) return res.status(404).json({ error: "Reviewable appeal not found." });
  res.json({ ok: true });
});
...
io.use((socket, next) => {
  if (blacklisted("network", networkFingerprintValue(socket.handshake.address))) return next(new Error("Access denied."));
  next();
});
